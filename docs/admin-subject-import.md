...
관리자 화면 `/admin/import-review`에서 통합정보시스템 강의계획서 JSON 또는 종합강의시간표 Excel을 현재 DB와 비교하고, 선택한 과목만 반영할 수 있다.

## 운영 순서

1. 통합정보시스템에서 내려받은 `INU_강의계획서_연도_학기.json` 또는 `종합강의시간표.xlsx`를 업로드한다.
...
변경 유형은 강의 추가, 폐강, 시간 변경, 강의실 변경, 재개설, 기타 정보 변경으로 나뉜다.
강의계획서 첨부 여부는 시간표 서비스에서 사용하지 않으므로 비교 및 DB 반영 대상에서 제외한다.
...
- 비교 정책이 바뀌기 전에 만든 미리보기는 `409 Conflict`로 중단하며 파일 재업로드를 요구한다.
- 파일의 년도·학기 컬럼 또는 제목행에서 확인한 학기가 선택한 반영 학기와 다르면 중단한다.
- 여러 학기 데이터가 한 Excel 파일에 섞여 있으면 중단한다.
